...
# Write me an endpoint for getting a list of chat histories
@router.get("/chat/history/", response_model=ChatHistoryOutput)
def chat_history(chat_history_guid: str = None):
    """
    Endpoint to get a list of chat histories.
    """
    response = handle_chat_history(chat_history_guid)
    return response
...
# === Web Scraper Endpoint ===
@router.post("/load-web-document/", response_model=WebDocumentLoaderResponse)
async def scrape_endpoint(data: WebDocumentLoaderRequest):
    """
    Endpoint to initiate the web document loading process.

    This asynchronous function receives a URL, passes it to the scrape handler,
    and returns the result of the web document loading process.

    Args:
    data (ScrapeRequest): The data containing the URL to be scraped.

    Returns:
    WebDocumentLoaderResponse: The result of the web document loading process encapsulated in a ScrapeResponse object.
    """
    # Delegate to the scrape handler and return the response.
    return await handle_web_doc_load(data)
...
async def process_documents_endpoint(
    ...
    """
    return await handle_process_documents(data)
